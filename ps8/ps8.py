...
class ResistantVirus(SimpleVirus):
    ...
    def switchResistances( self ):

        """
        switch the value in resistance dictionary.
        e.g. False to True
        True to False
        """
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            if self.isResistantTo( drug ):
                any_resist = True
                break
        # if there is no activeDrugs, means that ...
        if activeDrugs == []:
            any_resist = True
        if any_resist == False:
            ...
            if not random.random() < self.maxBirthProb *( 1.- popDensity ):
                ...
                offsprin = ResistantVirus( self.maxBirthProb , self.clearProb , \
                        self.resistances , self.mutProb )

                offsprin.switchResistances()
                return offsprin
...
class Patient(SimplePatient):
    ...
    def update(self):
        ...
        self.viruses = new_viruses_list

        pop_density = float(self.getTotalPop())/self.maxPop
        tmplist = []
        for viruse in self.viruses:
            try:
                tmplist.append( viruse.reproduce( pop_density , self.drugs ) )
...
def simulationWithDrug( numViruses , maxPop , maxBirthProb , clearProb , resistances , mutProb , numTrial):
    ...
    for _ in xrange( numTrial ):
        ...
        for i in xrange( 300 ):
            ...
            ave_viruse[i]        += patient.update()
            ave_resist_viruse[i] += patient.getResistPop( ['guttagonol'] )
    for i in xrange( 300 ):
        ave_viruse[i] /= numTrial
        ave_resist_viruse[i] /= numTrial

    pylab.plot( xrange(300) , ave_resist_viruse, 'bo' )
    pylab.xlabel( 'time step' )
    pylab.ylabel( 'average total viruses number' )
    pylab.show()
